...
public partial class Customers : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            double allPaid = amountPaying + paid;

            double newBalance = initialValue - allPaid;


            string sql = @"Update [PaymentVoucherCreditor] set Status = @status, [ReceivedBy] = @receivedBy,
            [DateReceived] = GETDATE()  where [PaymentVoucherID] = @voucherNo";
            SqlCommand cmd = new SqlCommand(sql, appconSQL2);
            cmd.Parameters.AddWithValue("@status", "Paid");
            cmd.Parameters.AddWithValue("@receivedBy", txtFullname.Text.Trim());
            ...
            Response.Redirect("VoucherPrintCreditor.aspx?id= " + lblVoucherNo.Text.Trim());
