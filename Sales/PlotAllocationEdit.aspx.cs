...
public partial class ViewSite : System.Web.UI.Page
{
    ...
    public DataTable LoadSelectedPlots()
    {
        ...
        {
            connection.Open();
            SqlCommand command = new SqlCommand(@" SELECT P.PlotNo, P.NormalPrice, 
            C.Fullname, P.AgreedPrice,  P.AmountPaid, P.Balance FROM Plots AS P JOIN Clients AS C ON P.OfferedTo = C.ClientNo
    ...
    public void LoadPlots()
    {
        ...
        if (generalReport.Rows.Count > 0)
        {
            ...
            foreach (DataRow row in generalReport.Rows)
            {
                ...
                sb.Append("<td>");
                sb.Append("<a href='PlotAllocationEditDetails.aspx?id=" + plotNo + "' class='btn btn-primary btn-sm'> Edit Offer</a>");
                sb.Append("</td>");
