...
public partial class ViewSite : System.Web.UI.Page
{
    ...
    public DataTable LoadSelectedPlots()
    {
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            SELECT  P.ID, P.PlotNo,
            C.Fullname, P.AgreedPrice,  P.AmountPaid, P.Balance, P.DateOffered FROM 
			Plots AS P LEFT JOIN 
			Clients AS C ON P.OfferedTo = C.ClientNo
		
            WHERE P.SiteNo = @siteNo and PlotStatus = @status
			AND P.PlotNo NOT IN (Select PlotNo from SaleAgreements)",  connection);
    ...
    public void LoadPlots()
    {
        ...
        if (generalReport.Rows.Count > 0)
        {
            ...
            foreach (DataRow row in generalReport.Rows)
            {
                ...
                sb.Append("<td>");
                sb.Append("<a href='ConfirmAgreement.aspx?id=" + plotNo + "' class='btn btn-primary btn-sm'> Sale Agreement</a>");
                sb.Append("</td>");
