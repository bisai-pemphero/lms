...
public partial class ViewSite : System.Web.UI.Page
{
    ...
    public DataTable LoadSelectedPlots()
    {
        ...
        {
            connection.Open();
            SqlCommand command = new SqlCommand(@"  Select P.ID, P.PlotNo, P.NormalPrice, C.Fullname, 
            P.AgreedPrice, P.AmountPaid, P.Balance, P.DateOffered from Plots AS P
		LEFT Join Clients as C on P.OfferedTo = C.ClientNo where P. SiteNo = @siteNo",
                connection);
    ...
    public void LoadPlots()
    {
        ...
        if (generalReport.Rows.Count > 0)
        {
            ...
            foreach (DataRow row in generalReport.Rows)
            {
                ...
                sb.Append("<td>");
                sb.Append("<a href='ViewSketch.aspx?id=" + Id + "' class='btn btn-primary btn-sm'> View Sketch</a>");
                sb.Append("</td>");
        ...
        else
        {
            ...
            allPlotsPlaceholder.InnerHtml = "<tr><td colspan='7'>No data found.</td></tr>";
        }
    }
