...
public partial class ViewCustomer : System.Web.UI.Page
{
    ...
    public void LoadBankDetails(string id)
    {
        string sql = "Select [BankName], [AccountNumber] from Sites where ID = @id";
        SqlCommand cmd = new SqlCommand(sql, appconSQL2);
        cmd.Parameters.AddWithValue("@id", id);
        SqlDataReader dr = cmd.ExecuteReader();
        while (dr.Read())
        {
            txtBank.Text = dr["BankName"] != DBNull.Value ? dr["BankName"].ToString() : "-";
           
            txtAccountNo.Text = dr["AccountNumber"] != DBNull.Value ? dr["AccountNumber"].ToString() : "-";
        }
    ...
    public void LoadCustomerDetails()
    {
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            SqlCommand command = new SqlCommand(@"Select [PaymentVoucherID], [PayeeName], [Description], [Amount], [Status] FROM [PaymentVoucherCreditor] where [SiteCode] = '"+lblSitecode.Text.Trim()+"'", connection);

            SqlDataReader reader = command.ExecuteReader();
            while (reader.Read())
            {
                string Id = reader["PaymentVoucherID"].ToString();
                string status = reader["status"].ToString();

                Response.Write("<tr>");
                Response.Write("<td>" + reader["PayeeName"] + "</td>");
                Response.Write("<td>" + reader["Description"] + "</td>");
                Response.Write("<td>" + reader["Amount"] + "</td>");
                Response.Write("<td>" + reader["Status"] + "</td>");

                Response.Write("<td>");
                Response.Write("<a href='CreditorVoucherEdit.aspx?id=" + Id + "' class='btn btn-primary btn-sm'> Edit</a>");
                Response.Write("</td>");

                Response.Write("<td>");
                if (status.Equals("Paid", StringComparison.OrdinalIgnoreCase))
                {
                    Response.Write("<a href='VoucherPrintCreditor.aspx?id=" + Id + "' class='btn btn-secondary btn-sm'>Print</a>");
                }
                Response.Write("</td>");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(hfInWords.Value))
            {
            ...
            string sql3 = @"Select * from [PaymentVoucherCreditor] where [SiteCode] = @siteCode and PayeeName = @name 
            ...
            else
            {
                ...
                dr3.Dispose();
                    ...
                    //insert into   Voucher register
                    string sql = @"Insert into [PaymentVoucherCreditor] ([PayeeName], [SiteCode], [Amount], 
                    [AmountInWords], [PaymentMethod] ,[ChequeNo], [BankName], [AccountNo],
                    [Description], [ReferenceNo], [PreparedBy])
	                VALUES (@name, @siteCode, @amount, @inwords, @method, @chequeNo, @bankName, 
                    @accountNo, @description, @referenceNo, @preparedBy)";
                    SqlCommand cmd = new SqlCommand(sql, appconSQL2);
                    cmd.Parameters.AddWithValue("@name", txtFullname.Text.Trim());
                    cmd.Parameters.AddWithValue("@amount", amount);
                    cmd.Parameters.AddWithValue("@inwords", hfInWords.Value);
