...
public partial class ViewSite : System.Web.UI.Page
{
    ...
    public DataTable LoadSelectedPlots()
    {
        ...
        {
            connection.Open();
            SqlCommand command = new SqlCommand(@"SELECT STUFF((
    SELECT ', ' + P2.PlotNo
    FROM SaleAgreementPlots P2
	Join SaleAgreements S ON S.Id = P2.SaleAgreementId
    WHERE P2.SiteNo = @siteNo
     
    FOR XML PATH(''), TYPE
).value('.', 'NVARCHAR(MAX)'), 1, 2, '') AS PlotNumbers, C.Fullname, COP.AmountPaid, SAP.SaleAgreementId 
 ...
 ",  connection);
    ...
    public void LoadPlots()
    {
        ...
        if (generalReport.Rows.Count > 0)
        {
            ...
            {
                string saId = row["SaleAgreementId"].ToString();
                sb.Append("<tr>");
                sb.Append("<td>" + row["PlotNumbers"] + "</td>"); 
                sb.Append("<td>" + row["Fullname"] + "</td>");
                sb.Append("<td>" + row["AmountPaid"] + "</td>");
                sb.Append("<td>");
                sb.Append("<a href='SelectCustomerOwnership.aspx?id=" + saId + "' class='btn btn-primary btn-sm'> Change of Ownership</a>");
                sb.Append("</td>");
