...
public partial class ViewSite : System.Web.UI.Page
{
    ...
    public DataTable LoadSelectedPlots()
    {
        ...
        {
            connection.Open();
            SqlCommand command = new SqlCommand(@"Select ID, PlotNo, NormalPrice from Plots where SiteNo = @siteNo
			 and OfferedTo = 'none'",
                connection);
    ...
    public void LoadPlots()
    {
        ...
        if (generalReport.Rows.Count > 0)
        {
            ...
            foreach (DataRow row in generalReport.Rows)
            {
                ...
                string plotId = row[0].ToString();
                string plotNo = row[1].ToString();

                sb.Append("<tr>");
                sb.Append("<td>" + row["PlotNo"] + "</td>");
                sb.Append("<td>" + row["NormalPrice"] + "</td>");
                sb.Append("<td>");
                sb.Append("<a href='EditPlotprice.aspx?id=" + plotId + "' class='btn btn-block btn-secondary btn-sm'> Adjust Price</a>");
                sb.Append("</td>");

                sb.Append("<td>");
                sb.Append("<a href='SelectCustomer.aspx?id=" + plotNo + "' class='btn btn-block btn-primary btn-sm'> Allocate</a>");
                sb.Append("</td>");
    ...
    {
        Response.Redirect("PlotAllocationEdit.aspx");
    }
}