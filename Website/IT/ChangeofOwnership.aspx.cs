...
public partial class ViewSite : System.Web.UI.Page
{
    ...
    public DataTable LoadSelectedPlots()
    {
        ...
        {
            connection.Open();
            SqlCommand command = new SqlCommand(@" SELECT COP.PlotNo, C.Fullname, COP.AmountPaid 
    ...
    public void LoadPlots()
    {
        ...
        if (generalReport.Rows.Count > 0)
        {
            ...
            foreach (DataRow row in generalReport.Rows)
            {
                ...
                sb.Append("<td>");
                sb.Append("<a href='SelectCustomerOwnership.aspx?id=" + plotNo + "' class='btn btn-primary btn-sm'> Change of Ownership</a>");
                sb.Append("</td>");
