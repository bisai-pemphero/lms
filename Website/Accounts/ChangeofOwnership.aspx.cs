...
public partial class ViewSite : System.Web.UI.Page
{
    ...
    public void LoadSites()
    {
        drpSite.Items.Clear();
        drpSite.Items.Add("");
        string sql = @"  Select Distinct SAP.[SiteNo], S.PhysicalLocation from SaleAgreementPlots as SAP 
  Join SaleAgreements as SA on SAP.SaleAgreementId = SA.Id
  Join Sites as S on SAP.SiteNo = S.SiteCode Order by S.PhysicalLocation";
        SqlCommand cmd = new SqlCommand(sql, appconSQL2);
    ...
    public DataTable LoadSelectedPlots()
    {
        ...
        {
            connection.Open();
            SqlCommand command = new SqlCommand(@"Select Distinct P.ID, P.PlotNo,
            C.Fullname, P.AgreedPrice,  P.AmountPaid, P.Balance, P.DateOffered from SaleAgreementPlots as SAP 
  Join SaleAgreements as SA on SAP.SaleAgreementId = SA.Id
  Join Plots as P on SAP.PlotNo = P.PlotNo
  Join Clients as C on SA.ClientNo = C.ClientNo 
  Where SAP.SiteNo = @siteNo ",  connection);
    ...
    public void LoadPlots()
    {
        ...
        if (generalReport.Rows.Count > 0)
        {
            ...
            foreach (DataRow row in generalReport.Rows)
            {
                ...
                sb.Append("<td>");
                sb.Append("<a href='ChangeofOwnershipPayments.aspx?id=" + plotNo + "' class='btn btn-block btn-primary btn-sm'> Payment </a>");
                sb.Append("</td>");
                sb.Append("</tr>");
            }
