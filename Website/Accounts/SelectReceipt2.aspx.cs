...
public partial class ChangeofOwnershipReceipt : System.Web.UI.Page
{
    string appconStr;
    ...
    public DataTable LoadSelectedPlots()
    {
        ...
        {
            connection.Open();
            SqlCommand command = new SqlCommand(@" SELECT COP.[ReceiptNo], C.Fullname, COP.AmountPaid 
                    FROM ChangeofOwershipPayments AS COP 
                    LEFT JOIN Clients AS C ON COP.PaidBy = C.ClientNo
					Join SaleAgreementPlots as SAP on SAP.SaleAgreementId = COP.SaleAgreementId
    ...
    public void LoadPlots()
    {
        ...
        if (generalReport.Rows.Count > 0)
        {
            ...
            foreach (DataRow row in generalReport.Rows)
            {
                ...
                sb.Append("<td>");
                sb.Append("<a href='OwnershipReceipt.aspx?id=" + receiptNo + "' class='btn btn-block btn-primary btn-sm'> View Receipt </a>");
                sb.Append("</td>");
                sb.Append("</tr>");
            }
        ...
        else
        {
            ...
            allPlotsPlaceholder.InnerHtml = "<tr><td colspan='7'>No receipts Found.</td></tr>";
        }
    }
