    ...
    public DataTable LoadSelectedPlots()
    {
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            SqlCommand command = new SqlCommand(@" SELECT P.ID, P.PlotNo, 
                    ...
                    P.AmountPaid, P.Balance, P.DateOffered 
                    FROM Plots AS P 
                    LEFT JOIN Clients AS C ON P.OfferedTo = C.ClientNo
                    WHERE P.SiteNo = @siteNo AND P.AmountPaid > 0",  connection);
    ...
    public void LoadPlots()
    {
        ...
        if (generalReport.Rows.Count > 0)
        {
            ...
            foreach (DataRow row in generalReport.Rows)
            {
                ...
                sb.Append("<td>");
                sb.Append("<a href='ViewReceiptPayments.aspx?id=" + plotNo + "' class='btn btn-block btn-primary btn-sm'> View Payments </a>");
                sb.Append("</td>");
                sb.Append("</tr>");
            }
