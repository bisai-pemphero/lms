...
public partial class ViewSite : System.Web.UI.Page
{
    ...
    public DataTable LoadSelectedPlots()
    {
        ...
        {
            connection.Open();
            SqlCommand command = new SqlCommand(@"   Select  CO.ProofofPayment, C.[Fullname], 
            S.PhysicalLocation, CO.Plot_Number from Clients as C 
            Join ChangeofOwnership as CO  on CO.Current_Owner = C.ClientNo
            Join Sites as S on CO.Site_Number = S.SiteCode
            where CO.Site_Number = @siteNo", connection);
    ...
    public void LoadPlots()
    {
        ...
        if (generalReport.Rows.Count > 0)
        {
            ...
            {
                string id = row["ProofofPayment"].ToString();

                sb.Append("<tr>");
                sb.Append("<td>" + row["Fullname"] + "</td>");
                sb.Append("<td>" + row["PhysicalLocation"] + "</td>");
                sb.Append("<td>" + row["Plot_Number"] + "</td>");

                sb.Append("<td>");
                sb.Append("<a href='ChangeofOwnershipDocument.aspx?id=" + id + "' class='btn btn-primary btn-sm'> View Change of Ownership</a>");
                sb.Append("</td>");
