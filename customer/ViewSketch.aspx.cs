...
public partial class ViewSite : System.Web.UI.Page
{
    ...
    {
        string sketch = "";
        string sql = @"SELECT PlotSketch FROM Plots WHERE ID = @Id";
        ...
        using (SqlCommand cmd = new SqlCommand(sql, appconSQL2))
        {
            ...
            try
            {
                using (SqlDataReader dr = cmd.ExecuteReader())
                {
                    if (dr.Read())
                    {
                        sketch = dr["PlotSketch"] != DBNull.Value ? dr["PlotSketch"].ToString() : "";
                ...
                else
                {
                    ...
                    pnlDocumentViewer.Controls.Add(new LiteralControl(@"
                        <div class='document-placeholder'>
                            ...
                            <p class='text-muted'>No sketch has been uploaded for this plot yet.</p>
                        </div>
                    "));
                }
            }
            ...
            {
                lblError.Text = "Error loading document: " + ex.Message;
            }
        }
    }
