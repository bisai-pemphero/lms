...
public partial class OfferLetter : System.Web.UI.Page
{
    ...
    public void LoadPlotDetails(string plotNo)
    {
        ...
        while (dr.Read())
        {
            lblPlotNo1.Text = dr.IsDBNull(0) ? string.Empty : dr.GetString(0);
            lblRefNo.Text = dr.IsDBNull(0) ? string.Empty : dr.GetString(0);
            lblPlotNo2.Text = dr.IsDBNull(0) ? string.Empty : dr.GetString(0);

            lblPlotSize.Text = (dr.IsDBNull(1) ? "0" : dr.GetString(1)) + " Square Meters";
            lblSite.Text = dr.IsDBNull(2) ? string.Empty : dr.GetString(2);
            lblSite2.Text = dr.IsDBNull(2) ? string.Empty : dr.GetString(2);

            double agreedPrice = dr.IsDBNull(3) ? 0.0 : dr.GetDouble(3);
            lblAgreedPrice.Text = "MK" + agreedPrice.ToString("N2");

            lblDated.Text = dr.IsDBNull(4) ? string.Empty : dr.GetString(4);
            lblCategory.Text = dr.IsDBNull(5) ? string.Empty : dr.GetString(5);

            int period = dr.IsDBNull(6) ? 0 : Convert.ToInt32(dr.GetString(6));
            lblPeriod.Text = period + " Months";

            string input = dr.IsDBNull(3) ? "0" : dr.GetDouble(3).ToString();
    ...
    protected string ConvertNumberToWords(long number)
    {
        ...
        if (number > 0)
        {
            ...
            {
                words += teensMap[number - 11]; // now safe
            }
            else
            {
        ...
        return words.Trim(); // ✅ clean output
    }
